...
function add_dropdowns() {
  let family_div = create_dropdown(
    "family",
    "Family",
    family_options,
    entry.family
...
function handle_discard() {
  $("#edit-entry-discard").click(function () {
    var result = confirm("Are you sure you want to discard?");
    if (result) {
      window.location.href = `/view/${entry.id}`;
    }
  });
...
$(document).ready(function () {
  $("#edit-entry-title").focus();
  global_architectures = entry.architectures;
  set_rating(entry);
  set_arches(global_architectures);
